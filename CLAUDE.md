...
| OpenAI | AI API（テキスト + 画像生成） | https://platform.openai.com |
...
### Step 3 — OpenAI セットアップ

1. https://platform.openai.com でアカウント作成
2. **API keys** ページで Secret key を発行（`sk-...`）
3. 使用モデルを決める
   - テキスト: `OPENAI_TEXT_MODEL`（デフォルト推奨 `gpt-4o-mini`）
   - 画像  : `OPENAI_IMAGE_MODEL`（デフォルト推奨 `gpt-image-2`）
...
| `ENCRYPTION_KEY` | 32 byte hex（暗号化キー、Vercel と同値） |
...
gh secret set ENCRYPTION_KEY --body "$(node -e 'console.log(require(\"crypto\").randomBytes(32).toString(\"hex\"))')"
```
...
- **AI**: OpenAI (テキスト `gpt-4o-mini` + 画像 `gpt-image-2`)
...
- `scripts/modules/`
  - `scraper.py` -- X トレンド収集 (X API V2 Recent Search) + AI トレンド分析 (OpenAI)
  - `generator.py` -- X 投稿文生成 (OpenAI Chat Completions)
  - `image_gen.py` -- 投稿サムネ画像生成 (OpenAI Images API, `gpt-image-2` 既定)
  - `x_poster.py` -- X へのツイート投稿 + media upload (X API V2 + v1.1 / tweepy、`X_CLIENT` で切替)
  - `discord_notify.py` -- Discord webhook 通知
  - `db.py` -- Supabase DB 操作
  - `crypto_helper.py` -- 認証情報の AES-256-GCM 復号 (ENCRYPTION_KEY 共有)
...
GitHub Actions cron
  ...
  → worker.py --cycle morning|night
      for each active account:
        1. scraper.run()       -- X トレンド収集 + OpenAI で trend 抽出
        2. generator.run()     -- X 投稿文生成 (OpenAI, 140字以内)
        3. image_gen.generate_image() -- サムネ画像生成 (OpenAI gpt-image-2)
        4. x_poster.post_tweet(media_bytes=...) -- X に投稿 + media upload
        5. discord_notify.tweet_done() -- Discord 通知
```
...
- `accounts` -- X アカウント情報 (x_username + X API V2 5点 / x_password_enc, genre_id, post_interval_minutes)
- `posts` -- 投稿履歴 (tweet_text, x_tweet_id, status: queued/posted/failed, image_prompt, has_image)
...
- GitHub Actions Secrets: `SUPABASE_URL`, `SUPABASE_KEY`, `OPENAI_API_KEY`, `OPENAI_TEXT_MODEL`, `OPENAI_IMAGE_MODEL`, `ENCRYPTION_KEY`,
  （任意）`X_BEARER_TOKEN`

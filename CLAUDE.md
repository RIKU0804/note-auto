...
3. （旧バージョンからアップグレードする場合のみ）`supabase/002_remove_note.sql` も実行して
   note.com 関連の旧カラムを削除する
...
- `scripts/modules/`
  - `scraper.py` -- X トレンド収集 (Playwright) + AI トレンド分析
  - `generator.py` -- X 投稿文生成 (OpenRouter)
  - `x_poster.py` -- X へのツイート投稿 (Playwright)
...
- アカウント間に 15〜20 分のランダム待機を挟む（凍結対策）
- Xアカウントのパスワードは `x_password_enc` に暗号化保存

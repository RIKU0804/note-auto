...
# TODO: from modules.reply_handler import run as reply_handler_run
# TODO: from modules.supabase_client import get_active_users, get_accounts_for_user, save_post
...
async def run_reply_loop():
    """
    Infinite loop: check and process replies for all users/accounts,
    then wait 25-35 minutes (base 30 +/- 5) before repeating.
    """
    logger.info("=== Starting reply loop ===")
        ...
        try:
            ...
            for user in users:
                ...
                for account in accounts:
                    account_name = account.get("name", "unknown")
                    try:
                        # TODO: count = await reply_handler_run(user, account)
                        count = 0  # placeholder
                        logger.info(
                        ...
                        await discord_notify.error(
                            user, "worker.run_reply_loop", str(e), account
                        )

        except Exception as e:
            logger.exception(f"[reply] Top-level error in reply loop: {e}")

        wait_minutes = REPLY_CHECK_BASE_MINUTES + random.randint(
            -REPLY_CHECK_RANGE_MINUTES, REPLY_CHECK_RANGE_MINUTES
        )
        logger.info(f"[reply] Next check in {wait_minutes} minutes")
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["reply"],
        help="Run mode (reply = infinite reply-check loop)",
    )
    ...
    if args.cycle:
        ...
        logger.info("Worker started — reply loop mode")
        asyncio.run(run_reply_loop())
    else:
        parser.print_help()
        raise SystemExit(1)
